...
/*             Registered Copyright 1994 G.A. Korn             2/11/94 */
...
void print(j)                /* WRITE string or value of an expression */
  ...
  unsigned char  ch ; int  i, gisav, gisav1 ; double  accum1,accum2 ;
       ...
       if (j>0) outptr=noteptr ;              /* write to journal file */
          ...
          gi++ ;                                             /* skip # */
          if (warea[gi]=='#')          /* computer-readable data file! */
             { gi++ ; spare5=1 ; } else spare5=0 ;
          gitchn() ;
...
L100 :    gi=gisav1 ; fprintf(outptr,"%g",accum1) ;     /* write value */
          }                                         /* else expression */
