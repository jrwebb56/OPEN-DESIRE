/* qOPENF.C    file operations for DESIRE                               */
/*            Registered Copyright 2007  G.A. Korn   UNIX              */
...
/*   killed references to editflg, SPARE3 (old SOLARIS)     6/14/04    */
/*   rld and reload now default to .src      8/12/07  */
...
void readname(filext ext)        /* read file name and, if necessary, */
/* default extension into array fname */
{           /* CALLED BY writefile, unsave, old00  */
    int li, idot ;
...
void  old00(int i)                    /* "old" and "chain" statements */
{
    int  li, lj, len, nn ;
...
void new00(int i)   /* "new" and "NEW" statements: clear data, program,  etc.
		       i = 1 is for editors using NEW */
{
    ...
    else { readname(".src") ; strcpy(pname,fname) ;}    /* get new name */
    if (i==0) printf("PIC = %s\n\n",pname) ;
