...
/*            Registered Copyright 2008 G.A. Korn                      */
...
/*   change keep, keep+                12/26/05      */
/*   change PIC for new version         8/13/07      */
/**  corrections                        8/15/07      */
/*   changed /PIC to PIC                1/29/08     **/
/* ------------------------------------------------------------------------- */
...
void pic0(){                       /* display or change PIC */
	...
	readname(".src") ; strcpy(pname,fname) ;
        }
    printf("PIC = %s\n",pname) ;                         /* display PIC */
...
/*  1     0        2          4          -2     -3      -4         -1 */
void prfile(int i){    /* list+, list, keep+, keep, edit, bye, help, or sh */
...
L9:         if (fname[lj]=='.') lj++ ; if (fname[lj]=='.') lj++ ;
    ...
    else if (i==0 || i==4) {                         /* keep */
	    strcpy(fname,pname) ;
	    lj=0 ;     
/* fix extension; first take care of .\, ..\, etc. */
L99:        if (fname[lj]=='.') lj++ ; if (fname[lj]=='.') lj++ ;
    ...
    else { fdn=noteptr ; dlist(i) ; return ; }  
/* list to notebook ???? */
    } 
	/* end if no filespec */
  if (i==1 || i==2) readname(".lst") ; else readname(".src") ; /* file spec */
 strcpy(pname,fname) ;   /** does this  work?? */ /***  added 12/26/05 <****/       
	
L15: if ((fdn=fopen(fname,"w"))==NULL) derror(11) ;         /* connect */
 dlist(i) ;               
/**    fprintf(fdn,"\n/PIC \'%s\'\n",fname) ;       
	       needed for reload only ????
	       fprintf(fdn,"/--\n") ;         
	       for WINDOWS version!         **/
        
   if (fclose(fdn)==EOF) {clearerr(fdn) ; derror(95) ; }    /* disconnect */
	
   system("clear") ;
   printf("-- keep in %s\n",fname) ; 
   fprintf(noteptr,"-- keep in %s\n",fname) ;       /* journal entry */
   return ;
   }                                /* keep, keep+, list or list+ */
...
/* ------------------------------------------------------------------- */
 ...
 if (i== -1 || i== -4) {                           /* sh or help */
	...
	if (warea[gi]==lf || warea[gi]=='|') {      /** edit entire file **/
	    fprintf(fdn,"/NEW\n") ;     
	    /****> - needs /NEW  *****/   
	    fprintf(fdn,"PIC \'%s\'\n",pname) ;  /*** changed from /PIC ***/          
            }         
     else if (i== -3 || i== -5) derror(33) ; 
 
 /* NOTE: special dialogs require line numbers, do NOT work with ed */
	...
	else if (i== -2 || i == -3) dlist(1) ; 

/* edit or bye, save WITH line numbers */
    ...
    if (i== -2) {                                     /* ed: call kedit & */
	printf("       please wait .......\n") ;
	if (system("sh ./editcfg1")<0) derror(121) ;
        }
    
    else if (i== -5) {                             /* edit: call kedit & */
	if (system("sh ./editcfg1")<0) derror(121) ;       
        }
