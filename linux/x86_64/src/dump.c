...
/*             Registered Copyright 2010 G.A. Korn    original file 2/17/92 */
...
/*             removed spare5=0 in MATPRNT          7/26/05 */
/**	       changed MATPRT to create tab-delimited text files    9/12/10 **/
/* ------------------------------------------------------------------- */
...
void  MATPRT() {             /* prints an array without name labels */
    ...
    for (lk=2 ; lk<=dim ; lk++) { offset*=intval[lj+lk] ; } /* total no. of elements */
    ...
    if (p->symtype==-1) { lj=intval[lj+dim+1] ; } /* points to first REAL element */
    else if (p->symtype==-3) {
	   lj=intval[lj+dim+1] ;            /* points to first REAL element */
	   offset*=2 ; count1*=2 ;
       }
    else { lj+=dim+1 ; ind1=8 ; }
