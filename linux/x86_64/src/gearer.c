/* GEARER.C    Variable-order/variable-step Integration - LINUX/UNIX              */
/*      Registered Copyright 2010  G.A. Korn       - old version 1/13/92 */
...
/* changed to permit DTMAX setting           1/9/10  */
/* ------------------------------------------------------------------------ */
...
#define HHHMAX relval[mmm+7]				/* DTMAX *?   /******>  <****/
...
void PSET() {                               /* changed PEDERV and interface */
	...
	for (J=1 ; J<=N ; J++) {
	    ...
	    /* DIFFUN */            drun00(2) ; if (errnum>0) return ;
...
/* **********     NLOOP Y1[I]=Y[I,1];
   ...
   NLOOP SAVE1[I]=sderiv[I] ;                      ********* */
	...
	/*FLAG200=TRUE*/  do {            /* while (NQ==1) */
		...
		NLOOP relval[I]=Y[I][1] ;
		/* DIFFUN */            drun00(2) ; if (errnum) return ;
...
void  DRIVE() {                 /* CALLED BY srun(); USES STIFF(), INTERP() */
	...
	/***        <******/    
	     if (fabs(TOUT-TOUTP)*10 < HHHMAX) HMAX=fabs(TOUT-TOUTP)*10;
	       else HMAX=HHHMAX ;            
...
/***              if ((EPS<=0.0) etc....  - taken care of in SRUN.C
	...
	/* ++++++++++> */   switch(KGO) {
		...
		FLAG1=FLAG=TRUE ;
		break ;
	    case 2 : errnum=224 ; return ;
		/* KFLAG= -1; fabs(H)=HMIN */
	    case 3 : errnum=225 ; return ;  
               /* KFLAG= -2; requested error is too small */
	    case 4 : errnum=226 ; return ;    
                /* KFLAG= -3; corrector did not converge */
		/* ++++++++++> */       }
	/* switch */
	if (FLAG2==TRUE) {
            FLAG2=FALSE;
